...
enum HarborLeaf: Hashable, Sendable {
    ...
    /// A Herdr pane addressed by the public pane id. Herdr's terminal session
    /// control API carries raw ANSI frames over stdio, so Harbor can render it
    /// without embedding Herdr's client UI.
...
enum HarborAttachCommand {
    ...
    static func terminalName(for item: HarborDragItem) -> String {
        ...
        case .sessionTUI(let _, let tool, let sessionName, _):

    ...
    private static func herdrSessionInfo(
        ...
        for pane in panes {
            ...
            let agent = makeAgentInfo(
                kind: pane.agent ?? pane.agent_session?.agent ?? listedAgent?.agent ?? listedAgent?.agent_session?.agent,
                name: pane.name
                    ?? pane.display_agent
                ...
                state: pane.agent_status ?? listedAgent?.agent_status,
                message: pane.message ?? listedAgent?.message,
                priority: pane.priority ?? listedAgent?.priority
            )

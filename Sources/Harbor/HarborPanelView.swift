...
struct HarborTreeOutlineView: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, NSOutlineViewDataSource, NSOutlineViewDelegate, NSMenuDelegate {
        ...
        func apply(nodes: [HarborTreeNode]) {
            ...
            let firstBuild = self.nodes.isEmpty
            self.nodes = nodes
            structureSignature = nextStructure
            if firstBuild {
                // Harbor is a catalog, so show the complete hierarchy on the
                // first load. This matches Cloud's tree behavior and makes a
                // discovered terminal draggable without an extra disclosure
                // click. Later refreshes keep the user's explicit collapses.

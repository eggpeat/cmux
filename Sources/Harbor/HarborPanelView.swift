...
struct HarborTreeOutlineView: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, NSOutlineViewDataSource, NSOutlineViewDelegate, NSMenuDelegate {
        ...
        func apply(nodes: [HarborTreeNode]) {
            ...
            if !hasPresentedCatalog, !userChangedExpansion, hasAttachableContent {
                // Harbor is a catalog, so show the complete hierarchy when
                // the first probe result arrives. The first model update can
                // be a loading placeholder, so this cannot be tied only to
                // the first outline build. Later refreshes keep the user's
                // explicit collapses.
        ...
        func outlineView(_ outlineView: NSOutlineView, heightOfRowByItem item: Any) -> CGFloat {
            ...
            switch node.kind {
            case .host: return 34
            case .terminal(let _, _, let info):
